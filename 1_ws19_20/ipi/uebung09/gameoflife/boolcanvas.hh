...
public:
    ...
    void writePixel(int i, int j, bool value);
               ...
               int horPixels, int vertPixels)
{
    ...
    _pixels = new bool[horPixels * vertPixels];
...
void Canvas::writePixel(int i, int j, bool value)
{
...
{
    int *intpixels = new int[_horPixels * _vertPixels];
    for (int i = 0; i < _horPixels * _vertPixels; i++)
    {
        if (_pixels[i])
        {
            intpixels[i] = 255;
    ...
    write_pgm(intpixels, _horPixels, _vertPixels, filename);
}
