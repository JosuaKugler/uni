...
\begin{frame}{Homotopically standard}
    almost contact str. on $S^{2n-1}$ $\leftrightsquigarrow$ almost complex str. on $B^{2n}$
    \pause 
    \begin{itemize}
        \item[$\rightsquigarrow$] extend almost complex str. from contact hypersurfaces on $S^{2n-1}$ to $TB^{2n}$
        \pause
        \item[$\leftsquigarrow$] field of complex tangencies $TS \cap J(TS)$ induces almost contact str. on $S^{2n-1}$.
    \end{itemize}

    \medskip \pause

    All almost complex structures on the ball are homotopic
\end{frame}
