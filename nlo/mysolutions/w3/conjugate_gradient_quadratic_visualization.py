...
import numpy as np
from conjugate_gradient_quadratic import *
...
for preconditioner, label in preconditioners:
	# solve problem
	outputs.append(
		conjugate_gradient_quadratic(lambda r: np.linalg.solve(preconditioner,r), A, b, c, x0, cauchy_quadratic, options)
		)
	labels.append(label)
...
for i in np.arange(nSamples):
	...
	outputs.append(
		conjugate_gradient_quadratic(lambda r: np.linalg.solve(preconditioner,r), A, b, c, np.array((x0x,x0y)), cauchy_quadratic, options)
		)
