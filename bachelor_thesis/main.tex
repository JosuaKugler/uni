...
Let \(K\) be a number field and \(\mathcal{O}\) its ring of integers with residue field \(k\).
...
\begin{itemize}
    ...
    \item There is a unique surjection 
    \[ 
        \phi_\Sigma\colon R_\Sigma \twoheadrightarrow \mathbb{T}_\Sigma
    \] 
...
Now that we know that \(R_\Sigma = \mathbb{T}_\Sigma\), consider a representation 
\(\rho \colon G_\mathbb{Q} \to \GL_2(\mathcal{O})\) and its reduction \(\overline{\rho}\).
Suppose that \(\overline{\rho}\) satisfies properties A - D from \cref{lem:ADproperties}.
Then by the universal property of \(R_\Sigma\) we get an \(\mathcal{O}\)-algebra morphism
\[
    \mathbb{T}_\Sigma \xrightarrow[\sim]{\phi_\Sigma^{-1}} R_\Sigma \xrightarrow{\phi} \mathcal{O}  
\]
s.t. \(\rho_\Sigma^\mathrm{mod} \cong \phi_\Sigma^{-1} \circ \phi \circ \rho\).
Because of the third property of \(\mathbb{T}_\Sigma\) stated above,
one can now deduce that \(\rho\) must be modular and the modularity lifting lemma follows.
