...
The goal of this part is to give a brief overview of the proof of Fermat's Last Theorem and to explain where Wiles'
numerical criterion enters the game.
...
\paragraph{Elliptic curves and Modularity}
...
\begin{definition}[Frey-Hellegouarch-Curve]
    ...
    \]
\end{definition}
This curve has weird properties (in a sence that numerical invariants of this curve contradict several well-known 
conjectures with good evidential and theoretical support) and Frey conjectured that such a curve couldn't exist.
He suggested to prove this by associating a modular form to this curve and showing that this modular form couldn't exist.
The fact (back then still a conjecture) that one could "associate" a modular form to every elliptic curve over \(Q\) is known as
...
\end{theorem}
This conjecture is what was proven by Wiles in his famous proof.
(In the years until 1999 the semistability condition could be removed, so that nowadays
the theorem holds for all elliptic curves.)

\subsection{Modular forms, elliptic curves and modularity}
The term "modularity" comes from modular forms, so lets start with a
\begin{definition}[Modular form]
    %give a standard definition here, in particular mention the a_n values!
\end{definition}
%Show the different equivalent conditions for modularity
\paragraph{Modularity as an isogeny/ a morphism}

\paragraph{Modularity for L-functions}

\paragraph{Modularity of galois representations}
...
and in the limit we obtain a Galois representation
\[
    \rho_{E, p} \colon G_\mathbb{Q} \to \GL_2(\mathbb{Z}_p).
\]

\begin{proposition}%probably cite Cornell silverman stevens on this matter 
...
%analogously, we can associate a galois representation to each newform/modular form/whatever form
...
the scope of this thesis. For a reference, see %diamond & shurman section 9.5

A Galois representation \(\rho\) is modular if there is a new cuspform \(f\) of weight \(2\) such that
\[
    \overline{\rho_{E, p}} \cong \overline{\rho_f}.  
\]
In particular, an elliptic curve is modular if the associated Galois representation is modular.

%zusammenfassung modularität (siehe Satz 5.1 Theorem Cornell )
In total we have
\begin{theorem}
    An elliptic curve \(E/\mathbb{Q}\) is modular if it satisfies one of the following equivalent conditions.
    \begin{itemize}
        \item \dots
    \end{itemize}
\end{theorem}
...
\begin{theorem}[Ribet's theorem]
    Let \(f\) be a newform of weight \(2\) and level \(N\ell\) for a prime \(\ell\) s.t. \(\ell\not | N\).%what is a newform
    Suppose that the associated Galois representation \(\overline{\rho_f}\) is absolutely irreducible%how does this association work?
    and satisfies one of the following conditions.
    \begin{itemize}
        \item \(\overline{\rho_f}\) is unramified at \(\ell\)
        \item \(\ell = p\) and \(\overline{\rho_f}\) is flat at \(p\).
    \end{itemize}
    Then there is a newform of weight \(2\) and level \(N\) s.t.
    \[
        \overline{\rho_f} \cong \overline{\rho_g} %in what exact sence are they equivalent?
    \]    
\end{theorem}

By the modularity theorem we find a newform \(f\) of weight \(2\) such that
\[
    \overline{\rho_{E_{a,b,c}, p}} \cong \overline{\rho_f} %equality or equivalence?
\]
We need the semistability of \(E_{a,b,c}\) and the properties shown in %give reference
Also, for \(\rho_{E_{a,b,c}, p}\) we have the following additional results by %give reference
\begin{itemize}
    \item \(\rho_{E_{a,b,c}, p}\) is absolutely irreducible
    \item \(\rho_{E_{a,b,c}, p}\) is unramified outside \(2p\), it is flat at \(p\) and semistable at \(2\).
\end{itemize}
Therefore, we can apply Ribets theorem %give the details?
 and finally find a newform \(g\) of weight \(2\) and level \(2\) such that
\[
    \overline{\rho_{E_{a,b,c}, p}} \cong \overline{\rho_g}.    
\]
However, the only newform of weight \(2\) and level \(2\) is 0, so we obtain the desired contradiction.

\subsection{Wiles' proof of modularity: The 3-5 trick}
...
Wiles chose to proof version \textbf{add number from equivalent modularity definition} of modularity.
Therefore we take a closer look at the \(p\)-adic representation \(\rho_{E,p} \colon G_\mathbb{Q} \to \GL_2(\mathbb{Z}_p)\)
associated to an elliptic curve \(E/\mathbb{Q}\).
First we prove several properties of the \(\mod p\)-reduction,
\begin{lemma}
    There is a prime \(p\) such that the \(\mod p\) reduction of the galois representation \(\rho_{E,p}\)
    \[
        \overline{\rho_{E,p}} \colon G_\mathbb{Q} \to \GL_2(\mathbb{F}_p)
    \]
    satisfies the following properties.
    \begin{enumerate}[(A)]
        \item \(\det \overline{\rho_{E,p}} = \chi_p\)
        \item \(\overline{\rho_{E,p}}\) is semistable
        \item \(\overline{\rho_{E,p}}\) is absolutely irreducible
        \item \(\overline{\rho_{E,p}}\) is modular, and \(\overline{\rho_{E,p}}|_{G_\mathbb{Q}(\sqrt(-3))}\) is absolutely irreducible
    \end{enumerate}
\end{lemma}
\begin{proof}
    We have seen in %add reference
    that \(\rho_{E, p}\) has determinant \(\chi_p\). \textbf{Is \(\chi_p\) invariant under reduction?}
    As \(E\) is semistable we know that \(\rho_{E, p}\) is semistable %again, why is this true for the reduction?
    We now need to prove that there is a \(p\) s.t. \(\overline{\rho_{E,p}}\) is absolutely irreducible and modular.
    %here comes the 3-5 trick
    %cite langlands-tunnell?
\end{proof}

\subsection{Wiles' proof of modularity: The main theorem}

Let \(A\) be a ring of coefficients \textbf{i.e. noetherian, local, Dedekind, ...?} and \(k\) its quotient field.
The goal of this part of the proof is to show that all lifts
\[
    \rho\colon G_\mathbb{Q} \to \GL_2(A)  
\]
of any galois representations \(\rho_0\colon G_\mathbb{Q} \to \GL_2(k)\) satisfying certain conditions are modular.

%deformations in general, deformation types

%universal deformation ring
For each deformation type \(\mathcal{D}\), we can define a universal deformation ring \(R_\mathcal{D}\) together
with a universal deformation 
\[ 
    \rho_\mathcal{D}\colon G_\mathbb{Q} \to \GL_2(R_\mathcal{D}).
\]
%universal modular deformation ring
%once we've proven R = T, the result follows

\subsection{Wiles' proof of modularity: \texorpdfstring{\(R = T\)}{R = T} and the numerical criterion}
