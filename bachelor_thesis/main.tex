...
Define the category \(\mathcal{C}\) as the category of local complete noetherian rings with residue field \(k\) 
and let \(A \in \mathcal{C}\).
The goal of this part of the proof is to show that all lifts
\[
    \rho\colon G_\mathbb{Q} \to \GL_2(A)  
\]
of any galois representations \(\rho_0\colon G_\mathbb{Q} \to \GL_2(k)\) are modular 
if the lifts satisfy certain conditions.
...
\[
    \mathcal{T}_\Sigma \colon \mathcal{C} \to \mathrm{\mathbf{FiniteSets}}  
\]
that sends \(A \in \mathcal{C}\) to the set of cuspidal eigenforms \(f\) over \(K\)
...
\begin{corollary}\label{cor:surjectivity_phi}
    ...
    Then \(A \to B\) is surjective if and only if \[\# \phi_A \geq \# \phi_B.\]
\end{corollary}
...
\begin{proof}
    ...
    As \(\# \phi_A\) is finite, we can apply \cref{cor:surjectivity_phi} and because \(f\)
