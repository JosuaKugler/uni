...
Define the category \(\co\) as in section 2.2. (i.e. local complete noetherian \(\mathcal{O}\)-algbras with residue field \(k\)) 
and let \(A \in \co\).
...
\end{itemize}
For a reference, \cite[see][chapter 1, paragraph 7.2]{Cornell1997}
...
\paragraph{The universal modular deformation ring}
Fix a number field \(K\) and consider the functor
\[
    \mathcal{T}_\Sigma \colon \co \to \mathrm{\mathbf{FiniteSets}}  
\]
that sends \(A \in \co\) to the set of cuspidal eigenforms \(f\) over \(K\)
such that \(\rho_f\) is a deformation of type \(\Sigma\). %equivalent to the a_p = tr rhobar(frob) condition?
...
\[
    \begin{tikzcd}
        ...
        \hom(\mathbb{T}_\Sigma, A) = \mathcal{T}_\Sigma(A) \arrow[r, equals]& \{\text{cuspidal eigenforms \(f\) s.t. \(\rho_f\) is of type \(\Sigma\)}\}\arrow[u, hook]
    \end{tikzcd}
\]
...
If we prove \(R_\Sigma \cong \mathbb{T}_\Sigma\), then the injection of the sets on the RHS 
