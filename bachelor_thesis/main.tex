...
conjectures (such as Serre's conjecture that was actually proved in 2008. See next section) 
and Frey conjectured that such a curve couldn't exist.
...
That might be one of the reasons why Frey  considered they Frey curve \(E_{a,b,c}\) to be strange.
...
\begin{theorem}[Ribet's theorem]
    ...
    \]    
\end{theorem}
By the modularity theorem \cref{th:modularity} we find a newform \(f\) of weight \(2\) such that
\[
...
Wiles chose to proof version 1 of modularity as defined in \cref{th:eqcondmodularity}
Therefore we take a closer look at the \(p\)-adic representation 
\(\rho_{E,p} \colon G_\mathbb{Q} \to \GL_2(\mathbb{Z}_p)\)
...
\begin{proof}
    ...
    %semistability of E guarantees that \(\overline{\rho}_{E,p}\) is either surjective or reducible
    %for every prime \(p \geq 3\). Hence for \(p \geq 3\), absolute irreducibility of \(\overline \rho_{E,p}\) is
    %equivalent to irreducibility of \(\overline \rho_{E,p}\), and if \(p = 3\) this is equivalent to absolute
    %irreducibility of \(\overline{\rho}_{E,3}|_{G_{\mathbb{Q}(\sqrt{3})}}\)
    \textbf{Explain why absolute irreducibility of rhobar and of rhobarsqrt-3 
    are clear once modularity and irreducibility are shown.}
    ...
    \begin{itemize}
        ...
        However, by \cref{th:eqcondmodularity}, then \(\overline{\rho}_{E',5}\) and thereby also 
...
\subsection{Wiles' proof of modularity: The modularity lifting theorem}
...
We say that a deformation of \(\overline{\rho}\) is of type \(\Sigma\), if
\begin{itemize}
    \item \dots
\end{itemize}
...
Define the category \(\co\) as in section 2.1. Then consider the functor
\[
    \mathcal{R}_\Sigma \colon \co \to \mathrm{\mathbf{FiniteSets}}  
\]
that sends \(A \in \co\) to the set of deformations of \(\overline{\rho}\) of type \(\Sigma\).
Deformation theory by Mazur tells us that this functor is representable and we denote the representing object \(R_\Sigma\).
...
\[
    \mathcal{T}_\Sigma \colon \co \to \mathrm{\mathbf{FiniteSets}}  
\]
that sends \(A \in \co\) to the set of modular forms \(f\) \textbf{primitive?} s.t. \(\rho_f\) is of type \(\Sigma\)
\textbf{Maybe some conditions are missing here}.
We can explicitly construct the representing object that we will denote \(\mathbb{T}_\Sigma\).

%Hecke algebra T
Let \(f\) be a modular form \textbf{newform? cuspidal? weight 2?}
Fix a number field \(K\) and regard it as a subfield of \(\overline{\mathbb{Q}}_\ell\) 
...
It can be shown that \(\mathbb{T}_\Sigma\) represents the functor \(\mathcal{T}_\Sigma\).
We have the diagram
%show functors on the left side and sets with representations/modular forms on the right side
\[
    \begin{tikzcd}
        \hom(R_\Sigma, A) = \mathcal{R}_\Sigma(A) \arrow[r, equals]& \{\text{deformations } \rho \colon G_\mathbb{Q} \to \GL_2(A) \text{ of type }\Sigma\}\\
        \hom(\mathbb{T}_\Sigma, A) = \mathcal{T}_\Sigma(A) \arrow[r, equals]& \{\text{some modular forms with conditions for their representations}\}\arrow[u, hook]
    \end{tikzcd}
\]
Because of contravariance, we get a surjection \(R_\Sigma \twoheadrightarrow \mathbb{T}_\Sigma\).
%once we've proven R = T, the result follows
If we prove \(R_\Sigma \twoheadrightarrow \mathbb{T}_\Sigma\), then the injection of the sets on the RHS 
of the above diagram is a surjection as well and we find a modular form for every representation in the upper set.
This proves the modularity lifting theorem.
