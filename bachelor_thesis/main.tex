...
\begin{corollary}\textup{\cite[corollary 5.12]{darmon1995fermat}}
...
\begin{proposition}
    ...
    the induced morphism \(tilde f\colon \phi_A \to \phi_B\) is an isomorphism,
...
\end{lemma}
\begin{proof}
    The idea is to lift the isomorphism on the tangent spaces by extending \(\phi_A\) 
    to a complete intersection \(\tilde A\).
    By \cref{rem:tangentspace}, we know that
    \[
        \phi_A = \phi_U/(f_1, \dots, f_m)
    \]
    for some linear polynomials \(f_1, \dots, f_m\) without constant term and, as usual, \(U = \mathcal{O}[X_1, \dots, X_n]\).
    As \(\phi_U\) is generated by \(X_1, \dots, X_n\), we know that \(\ker \phi_U \to \phi_A = (f_1, \dots, f_m)\)
    is generated by less generators ??
\end{proof}
